...
public class ServerScreen extends javax.swing.JFrame {
    ...
    public static void main(String args[]) {
        ...
        // Create InputDialog take admin from user
        String name = JOptionPane.showInputDialog(null, "Enter admin name :", "Admin (Only A-Z and a-z)", JOptionPane.PLAIN_MESSAGE);

        name = name.trim();

        if (name == null) {                                     // if user clicks on "cancel" button of InputDialog                   
            mServerScreen.terminateWindow();                    // terminate the window
        } else {
            if (name.isEmpty() || name.equals("")) {            // if admin provided is Empty
                JOptionPane.showMessageDialog(null, "Admin should not be empty", "Invalid Admin", JOptionPane.ERROR_MESSAGE); // show dialog as "Invalid Admin"
                mServerScreen.terminateWindow();                // terminate the window
            } else {
                if (Utils.isAlpha(name)) {                      // check whether the admin provided is alphnumeric or not
                    ...
                    // if admin contains anything else than alphabates then show dialog as "Invalid Admin"
                    JOptionPane.showMessageDialog(null, "Only alphabets are allowed in admin (A-Z and a-z)", "Invalid Admin", JOptionPane.ERROR_MESSAGE);
                    mServerScreen.terminateWindow();            // terminate the window
                }
            }
        }
    ...
    public void sendMessageBasedOnMessageType(String user, String message) {
        ...
        if (MESSAGE_TYPE.equals("Unicast")) {
            ...
            for (HandleClient c : clients) {
                c.sendMessage(user, message);                                           //if message_type is Broadcast then send message to all online users
            }
        }
    }
